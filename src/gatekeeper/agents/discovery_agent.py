...
class DiscoveryAgent:
    ...
    async def get_product_url(self, product: ProductSchema) -> URL:
        ...
        return StoreUrlFactory.build_store_product_url(product.slug, path_type=path_type)

    @retry(max_attempts=3, wait=5)
    async def __get_product_path_type(self, slug: str) -> ProductPathType:
        for ptype in ProductPathType:
            await self.__page.goto(str(StoreUrlFactory.build_store_product_url(slug, ptype)), wait_until="domcontentloaded")
